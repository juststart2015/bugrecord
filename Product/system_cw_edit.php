	...
	if(isset($_POST["submit"]) && $_POST["submit"] == "保存")
		{
			...
			if($cljg == "" || $clr == "")
				{
					echo "<script>alert('请填写更新内容及处理人');location.href='system_cw.php'</script>";
				}
			else
				{
					...
					$sql = "update system_cw set zt='$zt',tcr='$tcr',djrq='$djrq',wtms='$wtms',cljg='$cljg',clr='$clr',wcrq='$wcrq',bz='$bz' where tcr='$tcr' and wtms='$wtms'";
					$sql_gx = "insert into system_cw_clgc (wtms,clgc,bjr) values ('$wtms','$cljg','$bjr')";
					...
					if(!$result)
						{
							echo "<script>alert('未保存，请重试');location.href='system_cw.php'</script>";
						}
					else
						{
							echo "<script>alert('保存成功,点击刷新');location.href='system_cw.php'</script>";
						}
				}
		}
	else
		{
			if(isset($_POST["submit"]) && $_POST["submit"] == "确认")
			{
				...
				if($assigned_name == "默认")
				{
					echo "<script>alert('指派失败，请选择指派对象');location.href='system_cw.php'</script>";
				} 
				else
				{
					$result = mysql_query($sql);
					...
					echo "<script>alert('指派成功，点击刷新');location.href='system_cw.php'</script>";
				}
			}
			else
			{
				...
				$sql_gb_a = "update system_cw set zt = '已完成' where wt_id = '$wt_id'";
				$sql_gb = "insert into system_cw_clgc (wtms,clgc,bjr) values ('$wtms','关闭此问题','$bjr')";
				...
				echo "<script>alert('问题已关闭');location.href='system_cw.php'</script>";
			}
		}
?>
